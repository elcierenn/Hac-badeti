...
export default function MuzdelifeVakfesiAdim1Screen() {
  const { t } = useTranslation();
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const bottomPad = 12 + insets.bottom;
  const k = 'muzdelifeVakfesiAdim1';
              ...
              <View style={styles.blurBlock}>
                <View style={styles.bubble}>
                  <ScrollView
                    ...
                    nestedScrollEnabled
                  >
                    <Text style={styles.sectionTitle}>{t(`${k}.sequenceTitle`)}</Text>
                    <Text style={styles.prayerFirst}>{t(`${k}.s1`)}</Text>
                    <Text style={styles.prayerItem}>{t(`${k}.s2`)}</Text>
                    <Text style={styles.prayerItem}>{t(`${k}.s3`)}</Text>
                    <Text style={styles.prayerItem}>{t(`${k}.s4`)}</Text>
                    <Text style={styles.prayerItem}>{t(`${k}.s5`)}</Text>
